...
class SearchableDataSources():
	def __init__(self):
		...
		self.search_results_dir_path = "pub_finder_runs/searches"
		self.search_results_file_name_latest = "latest.json"
	...
	# Save Over the 'latest.json' search results and save a json file which contains the run_id as the base part of the filename
	def save__search_results(self, search_results_list):
		...
		obj_to_save = {}
		obj_to_save['run_id'] 			= self.run_id
		obj_to_save['search_results'] 	= search_results_list
		...
		storage_instance.save_dict_to_json_file(data_dict=obj_to_save, directory_path=self.search_results_dir_path, file_name=self.search_results_file_name_latest) # latest.json

		# Create the filename for the current run_id and save that one as well.
		current_run_id__json_file_name = str(self.run_id) + ".json"
		storage_instance.save_dict_to_json_file(data_dict=obj_to_save, directory_path=self.search_results_dir_path, file_name=current_run_id__json_file_name) # <yyyy_mm_dd__hh_mm_ss>.json
	...
	def perform_searches(self, search_phrases, use_local_example_search_results_data=True):
		...
		# Save the search results as a new run and Save Over the 'latest.json' search results
		self.save__search_results(search_results_list=search_results)
